/*
	Author: Moldisocks
	Last Modified: 2020.04.28 08.51
...
{
	{
		...
		if (side _player == _zone_side) then {
			...
			{
				if ((_player inArea _x) and (_playerHeight <= [_x] call mld_fnc_sz_getHeight)) exitWith {
			...
			if (_inFriendlyArea) then {
				...
				[_currentfriendlyMarker] remoteExec ["mld_fnc_sz_safe",_player];
			};

			if (_inEnemyArea) then {
				//Start kill countdown
				[_currentEnemyMarker] remoteExec ["mld_fnc_sz_kill",_player];
			};
			...
			if (!_inEnemyArea and !_inFriendlyArea) then {
				...
				[] remoteExec ["mld_fnc_sz_neutral",_player];
			};
		};
	} forEach allPlayers;
