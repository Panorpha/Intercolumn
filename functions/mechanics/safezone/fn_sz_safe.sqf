/*
	Author: Moldisocks
	Last Modified: 2020.04.28 20.31
...
if (!sz_safezoneing and !sz_safezoned) then {
	...
	while {_i > 0} do
	{
		...
		_i = _i - 1;
		sleep 1;
		systemChat format["inArea: %1 | belowHeight: %2",(player inArea _friendly_marker),(((getPos player) select 2) <= [_friendly_marker] call mld_fnc_sz_getHeight)];
		if (!(player inArea _friendly_marker) or !(((getPos player) select 2) <= [_friendly_marker] call mld_fnc_sz_getHeight)) exitwith {_followThrough = true};
