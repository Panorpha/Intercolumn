/*
	Author: Moldisocks
	Last Modified: 2020.04.29 13.40
...
if (!sz_inEnemyZone) then {
	...
	while {_i > 0} do
	{
		...
		_i = _i - 1;
		sleep 1;
		systemChat format["inArea: %1 | belowHeight: %2",(player inArea _enemy_marker),(((getPos player) select 2) <= [_enemy_marker] call mld_fnc_sz_getHeight)];
		if (!(player inArea _enemy_marker) or !(((getPos player) select 2) <= [_enemy_marker] call mld_fnc_sz_getHeight)) exitwith {_followThrough = true};
